...
export function AppShell() {
  ...
  return <div className="min-h-screen bg-paper text-ink"><button className="fixed left-4 top-4 z-40 rounded-xl bg-ink p-2.5 text-white lg:hidden" onClick={() => setOpen(true)} aria-label="打开导航"><Menu size={20} /></button>{open && <button className="fixed inset-0 z-40 bg-ink/30 lg:hidden" aria-label="关闭导航遮罩" onClick={() => setOpen(false)} />}
