...
export const nodeInputSchema = z.object({ type: z.enum(nodeTypes), content: z.string().trim().min(1, '请输入节点内容').max(nodeContentMaxLength, `节点内容不能超过 ${nodeContentMaxLength.toLocaleString()} 个字符`), status: z.enum(nodeStatuses).nullable(), vulnerabilityStatus: z.enum(vulnerabilityStatuses).optional(), log: z.string().max(nodeLogMaxLength, `附加日志不能超过 ${nodeLogMaxLength.toLocaleString()} 个字符`) })
