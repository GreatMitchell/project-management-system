...
export const nodeStatuses = projectStatuses; export const nodeTypes = ['question', 'solution', 'result', 'assumption', 'vulnerability'] as const;
export const edgeTypes = ['exposes', 'patches', 'inherits', 'weakens'] as const;
export const vulnerabilityStatuses = ['unexplored', 'patched', 'open', 'conquered'] as const;
export const projectTypes = ['general', 'research'] as const;
export const validationResults = ['passed', 'partial', 'failed'] as const; export const reviewTriggers = ['milestone', 'project_end', 'manual'] as const
...
export interface PraxisNode { id: string; projectId: string; type: NodeType; content: string; status: NodeStatus; vulnerabilityStatus?: VulnerabilityStatus; log: string; position: number; createdAt: string; updatedAt: string }
...
export const projectInputSchema = z.object({ title: z.string().trim().min(1, '请输入项目名称').max(80), trigger: z.string().trim().min(1, '请记录启动项目的现实触发').max(1000), status: z.enum(projectStatuses), type: z.enum(projectTypes) })
export const nodeInputSchema = z.object({ type: z.enum(nodeTypes), content: z.string().trim().min(1, '请输入节点内容').max(2000), status: z.enum(nodeStatuses), vulnerabilityStatus: z.enum(vulnerabilityStatuses).optional(), log: z.string().max(5000) })
...
export type LegacyProject = Omit<Project, 'activeNodeId' | 'type' | 'focusedNodeIds'>; export type LegacyConnection = Omit<NodeConnection, 'isPreferred' | 'edgeType'>
