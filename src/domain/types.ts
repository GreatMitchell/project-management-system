...
export const nodeInputSchema = z.object({ type: z.enum(nodeTypes), content: z.string().trim().min(1, '请输入节点内容').max(2000), status: z.enum(nodeStatuses).nullable(), vulnerabilityStatus: z.enum(vulnerabilityStatuses).optional(), log: z.string().max(5000) })
