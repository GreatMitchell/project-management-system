...
export function TaskGraphNode({ data }: NodeProps<TaskFlowNode>) {
  ...
  return <article tabIndex={0} aria-label={`${Meta.label}节点：${item.content}。${data.presentation.label}${data.milestone ? `。里程碑：${data.milestone.title}` : ''}`} data-visual-state={data.presentation.state} data-milestone={data.milestone ? milestoneState : undefined} style={{ '--route-step': data.transitionIndex } as React.CSSProperties} className={`graph-node graph-node-kind-${item.type} graph-node-${data.presentation.state} ${data.selected ? 'graph-node-selected' : ''} ${data.onRoute ? 'graph-node-route' : ''} ${data.active ? 'graph-node-active' : ''} ${data.presentation.needsChoice ? 'graph-node-choice' : ''} ${data.milestone ? 'graph-node-milestone' : ''} ${data.transitioning ? 'graph-node-transitioning' : ''}`} onClick={() => data.onSelect(item)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); data.onSelect(item) } }}>
    <span className="graph-node-frame" aria-hidden="true" /><span className="tech-only graph-node-signal" aria-hidden="true"><i /><i /><i /></span><span className="game-only graph-node-seal" aria-hidden="true">{data.active ? 'CURRENT' : data.presentation.state === 'completed' || data.presentation.state === 'unlocked' ? 'CLEARED' : data.presentation.state === 'blocked' ? 'UNKNOWN' : data.presentation.state === 'abandoned' ? 'CLOSED' : ''}</span><span className={`game-only graph-outpost-emblem graph-outpost-${item.type}`} aria-hidden="true"><Meta.icon size={20} /></span><Handle type="target" position={Position.Left} className="graph-handle" />
    <div className="graph-node-header"><span className="tech-only graph-node-module-id" aria-hidden="true">{item.type === 'question' ? 'QRY' : item.type === 'solution' ? 'ACT' : 'OUT'} / {(item.position + 1).toString().padStart(2, '0')}</span><span className={`graph-node-type graph-node-type-${item.type}`}><Meta.icon size={17} />{Meta.label}</span><span className={`status-badge !px-2.5 !py-1 !text-[11px] ${statusStyles[item.status]}`}>{statusLabels[item.status]}</span></div>
    ...
    <div className="graph-node-footer"><span className="graph-node-index"><span className="game-only">{item.type === 'question' ? 'Q' : item.type === 'solution' ? 'S' : 'R'}-</span>{(item.position + 1).toString().padStart(2, '0')}</span><div className="graph-node-actions nodrag flex items-center gap-2">{data.collapsibleBranches.map((branch) => <button key={branch.id} className="graph-branch-toggle" aria-label={`${branch.collapsed ? '展开' : '折叠'}包含 ${branch.count} 个节点的支线`} title={`${branch.collapsed ? '展开' : '折叠'}支线`} onClick={(event) => { event.stopPropagation(); data.onToggleBranch(branch.id) }}><GitBranch size={13} />{branch.collapsed ? `展开 ${branch.count}` : `收起 ${branch.count}`}</button>)}{data.active && <button className="graph-advance-button" onClick={(event) => { event.stopPropagation(); data.onAdvance(item) }}><Plus size={14} />继续推进</button>}</div></div>
...
export function BranchSummaryNode({ data }: NodeProps<BranchSummaryFlowNode>) { return <button className="graph-branch-summary" aria-label={`展开已收起的支线，共 ${data.count} 个节点`} onClick={() => data.onExpand(data.branchId)}><span className="graph-branch-sign" aria-hidden="true"><GitBranch size={20} /></span><span className="theme-label-default">支线已收起</span><span className="game-only">迷雾岔路</span><strong>{data.count} 个节点</strong><Handle type="target" position={Position.Left} className="graph-handle" /></button> }
