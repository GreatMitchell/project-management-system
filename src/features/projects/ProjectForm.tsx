...
import { projectInputSchema, type Project, type ProjectInput } from '../../domain/types'
...
export function ProjectForm({ open, project, onClose, onSubmit }: Props) {
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<ProjectInput>({ resolver: zodResolver(projectInputSchema), defaultValues: { title: '', trigger: '', status: 'exploring' } })
  useEffect(() => { if (open) reset(project ? { title: project.title, trigger: project.trigger, status: project.status } : { title: '', trigger: '', status: 'exploring' }) }, [open, project, reset])
