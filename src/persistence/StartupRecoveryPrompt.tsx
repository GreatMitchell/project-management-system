...
export function StartupRecoveryPrompt() {
  ...
  const restore = async () => {
    ...
    } catch { notify('快照恢复失败，现有数据未被修改', 'error') }
  }
  return <Modal open={Boolean(snapshot)} title="发现可恢复的本地快照" description="当前浏览器中没有项目，但已授权目录里存在持久副本。" onClose={() => setSnapshot(null)}><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900"><strong>{snapshot?.projectCount} 个项目，{snapshot?.nodeCount} 个节点</strong><p>创建于 {snapshot && new Date(snapshot.createdAt).toLocaleString('zh-CN')}</p></div><p className="mt-4 text-sm leading-6 text-moss">恢复会先校验快照，然后写入当前为空的本地数据库。快照文件本身不会被修改。</p><div className="mt-6 flex justify-end gap-3"><button className="button-secondary" onClick={() => setSnapshot(null)}>暂不恢复</button><button className="button-primary" onClick={restore}>恢复最近快照</button></div></Modal>
}
