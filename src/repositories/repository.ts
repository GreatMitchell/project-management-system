...
import { buildLinearConnections, resolveCurrentRoute, validateConnectionSet, validateResearchConnectionSet } from '../domain/graph'
import { graphPositionSchema, milestoneInputSchema, nodeInputSchema, projectInputSchema, reviewInputSchema, type BackupData, type BackupDataV1, type BackupDataV2, type EdgeType, type GraphPosition, type MilestoneInput, type NodeConnection, type NodeCreateMode, type NodeInput, type PraxisNode, type Project, type ProjectBundle, type ProjectInput, type ProjectStatus, type ReviewInput } from '../domain/types'
...
export interface GraphPositionUpdate { nodeId: string; graphPosition: GraphPosition }
...
function validateProjects(projects: Project[], nodes: PraxisNode[], connections: NodeConnection[]) {
  const byProject = new Map<string, NodeConnection[]>()
  for (const conn of connections) { const list = byProject.get(conn.projectId) ?? []; list.push(conn); byProject.set(conn.projectId, list) }
  for (const project of projects) {
    const projectNodes = nodes.filter(n => n.projectId === project.id)
    const projectConns = byProject.get(project.id) ?? []
    if (project.type === 'research') validateResearchConnectionSet(projectNodes, projectConns)
    else validateConnectionSet(projectNodes, projectConns)
  }
...
export const repository = {
  async listProjects() { return (await db.projects.orderBy('updatedAt').reverse().toArray()).map((project) => ({ ...project, pinnedAt: project.pinnedAt ?? null })) },
  async getBundle(projectId: string): Promise<ProjectBundle | null> {
    const raw = await db.projects.get(projectId); if (!raw) return null
    const project: Project = { ...raw, focusedNodeIds: raw.focusedNodeIds ?? [], pinnedAt: raw.pinnedAt ?? null }
  ...
  async createProject(input: ProjectInput) { const clean = projectInputSchema.parse(input); const timestamp = now(); const project: Project = { id: id(), ...clean, activeNodeId: null, focusedNodeIds: [], pinnedAt: null, createdAt: timestamp, updatedAt: timestamp }; await db.projects.add(project); return project },
  ...
  async toggleProjectPin(projectId: string) { const project = await db.projects.get(projectId); if (!project) throw new Error('项目不存在'); await db.projects.update(projectId, { pinnedAt: project.pinnedAt ? null : now() }) },
  ...
  async saveNode(projectId: string, input: NodeInput, nodeId?: string, options: SaveNodeOptions = {}) {
    ...
    const node: PraxisNode = { id: id(), projectId, ...clean, status: project.type === 'research' && (clean.type === 'assumption' || clean.type === 'vulnerability') ? null : clean.status, position: nextNodePosition(nodes), createdAt: timestamp, updatedAt: timestamp }
    await db.transaction('rw', db.nodes, db.nodeConnections, db.projects, async () => { await db.nodes.add(node); if (predecessor) await db.nodeConnections.add({ id: id(), projectId, sourceNodeId: predecessor.id, targetNodeId: node.id, isPreferred: false, createdAt: timestamp }); const activeNodeId = project.type === 'general' && (nodes.length === 0 || mode === 'advance') ? node.id : project.activeNodeId; await db.projects.update(projectId, { activeNodeId, updatedAt: timestamp }) }); return node
  },
  ...
  async toggleFocusedNode(projectId: string, nodeId: string) { const project = await db.projects.get(projectId); if (!project) throw new Error('项目不存在'); const nodes = await db.nodes.where('projectId').equals(projectId).toArray(); if (!nodes.some((node) => node.id === nodeId)) throw new Error('节点不存在'); const focused = new Set(project.focusedNodeIds); if (focused.has(nodeId)) focused.delete(nodeId); else focused.add(nodeId); await db.projects.update(projectId, { focusedNodeIds: [...focused], updatedAt: now() }) },

  async createConnection(projectId: string, sourceNodeId: string, targetNodeId: string, edgeType?: EdgeType) {
    ...
    const connection: NodeConnection = { id: id(), projectId, sourceNodeId, targetNodeId, edgeType, isPreferred: false, createdAt: now() }; 
    if (project.type === 'research') validateResearchConnectionSet(nodes, [...connections, connection])
    else validateConnectionSet(nodes, [...connections, connection])
  ...
  async updateConnection(connectionId: string, edgeType?: EdgeType) { const connection = await db.nodeConnections.get(connectionId); if (!connection) throw new Error('连接不存在'); await db.transaction('rw', db.nodeConnections, db.projects, async () => { await db.nodeConnections.update(connectionId, { edgeType }); await db.projects.update(connection.projectId, { updatedAt: now() }) }) },
  ...
  async deleteNode(nodeId: string) { const node = await db.nodes.get(nodeId); if (!node) return; const bundle = await this.getBundle(node.projectId); const protectedNode = bundle?.project.type === 'research' ? bundle.project.focusedNodeIds.includes(nodeId) : Boolean(bundle && resolveCurrentRoute(bundle.nodes, bundle.connections, bundle.project.activeNodeId).nodeIds.includes(nodeId)); if (protectedNode) throw new Error(bundle?.project.type === 'research' ? '核心节点不能删除，请先取消核心标记' : '当前路线节点不能删除，请先切换路线或取消追踪'); await db.transaction('rw', db.nodes, db.nodeConnections, db.milestones, db.projects, async () => { await db.milestones.where('nodeId').equals(nodeId).delete(); await db.nodeConnections.where('sourceNodeId').equals(nodeId).delete(); await db.nodeConnections.where('targetNodeId').equals(nodeId).delete(); await db.nodes.delete(nodeId); const remaining = await db.nodes.where('projectId').equals(node.projectId).sortBy('position'); await Promise.all(remaining.map((item, position) => db.nodes.update(item.id, { position }))); await db.projects.update(node.projectId, { focusedNodeIds: (bundle?.project.focusedNodeIds ?? []).filter((id) => id !== nodeId), updatedAt: now() }) }) },
  ...
  async saveGraphPositions(projectId: string, updates: readonly GraphPositionUpdate[]) {
    const positions = new Map(updates.map((update) => [update.nodeId, graphPositionSchema.parse(update.graphPosition)])); if (!positions.size) return
    const nodes = await db.nodes.bulkGet([...positions.keys()]); if (nodes.some((node) => !node || node.projectId !== projectId)) throw new Error('只能保存同一项目中存在节点的路线图位置')
    await db.transaction('rw', db.nodes, async () => { await Promise.all([...positions].map(([nodeId, graphPosition]) => db.nodes.update(nodeId, { graphPosition }))) })
  },
  ...
  async importData(raw: unknown, mode: 'merge' | 'replace') {
    const base = z.object({ id: z.string(), createdAt: z.string(), updatedAt: z.string() }).passthrough(); const legacyProject = base.extend({ trigger: z.string(), title: z.string(), status: z.string() }); const currentProject = legacyProject.extend({ activeNodeId: z.string().nullable(), pinnedAt: z.string().nullable().optional() }); const node = base.extend({ projectId: z.string(), type: z.string(), content: z.string(), status: z.string(), log: z.string(), position: z.number().int().nonnegative(), graphPosition: graphPositionSchema.optional() }); const legacyEdge = z.object({ id: z.string(), projectId: z.string(), sourceNodeId: z.string(), targetNodeId: z.string(), createdAt: z.string() }); const currentEdge = legacyEdge.extend({ isPreferred: z.boolean() }); const milestone = base.extend({ projectId: z.string(), nodeId: z.string(), title: z.string(), method: z.string(), criteria: z.string(), result: z.string().nullable(), feeling: z.string(), validatedAt: z.string().nullable() }); const review = base.extend({ projectId: z.string(), trigger: z.string(), health: z.string(), execution: z.string(), systemAdjustment: z.string() }); const common = { exportedAt: z.string(), nodes: z.array(node), milestones: z.array(milestone), reviews: z.array(review) }
    const schema = z.discriminatedUnion('version', [z.object({ version: z.literal(1), ...common, projects: z.array(legacyProject) }), z.object({ version: z.literal(2), ...common, projects: z.array(legacyProject), connections: z.array(legacyEdge) }), z.object({ version: z.literal(3), ...common, projects: z.array(currentProject), connections: z.array(currentEdge) })]); const rawData = schema.parse(raw) as unknown as BackupData | BackupDataV1 | BackupDataV2
    ...
    const projects: Project[] = rawData.version === 3 ? rawData.projects.map((item) => ({ ...item, pinnedAt: item.pinnedAt ?? null })) : rawData.projects.map((project) => ({ ...project, type: 'general' as const, activeNodeId: [...rawData.nodes].filter((node) => node.projectId === project.id).sort((a, b) => b.position - a.position)[0]?.id ?? null, focusedNodeIds: [], pinnedAt: null }))
