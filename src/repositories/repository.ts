...
export const repository = {
  ...
  async createProject(input: ProjectInput) { const clean = projectInputSchema.parse(input); const timestamp = now(); const project: Project = { id: id(), ...clean, activeNodeId: null, focusedNodeIds: [], createdAt: timestamp, updatedAt: timestamp }; await db.projects.add(project); return project },
  ...
  async toggleFocusedNode(projectId: string, nodeId: string) { const project = await db.projects.get(projectId); if (!project || project.type !== 'research') throw new Error('只有科研项目支持重点关注'); const nodes = await db.nodes.where('projectId').equals(projectId).toArray(); if (!nodes.some((node) => node.id === nodeId)) throw new Error('重点关注节点不存在'); const focused = new Set(project.focusedNodeIds); if (focused.has(nodeId)) focused.delete(nodeId); else focused.add(nodeId); await db.projects.update(projectId, { focusedNodeIds: [...focused], updatedAt: now() }) },
  ...
  async importData(raw: unknown, mode: 'merge' | 'replace') {
    ...
    const projects: Project[] = rawData.version === 3 ? rawData.projects : rawData.projects.map((project) => ({ ...project, type: 'general' as const, activeNodeId: [...rawData.nodes].filter((node) => node.projectId === project.id).sort((a, b) => b.position - a.position)[0]?.id ?? null, focusedNodeIds: [] }))
