...
export const repository = {
  ...
  async createProject(input: ProjectInput) { const clean = projectInputSchema.parse(input); const timestamp = now(); const project: Project = { id: id(), ...clean, activeNodeId: clean.type === 'research' ? undefined : null, focusedNodeIds: clean.type === 'research' ? [] : undefined, createdAt: timestamp, updatedAt: timestamp }; await db.projects.add(project); return project },
  ...
  async importData(raw: unknown, mode: 'merge' | 'replace') {
    ...
    const projects: Project[] = rawData.version === 3 ? rawData.projects : rawData.projects.map((project) => ({ ...project, type: 'general' as const, activeNodeId: [...rawData.nodes].filter((node) => node.projectId === project.id).sort((a, b) => b.position - a.position)[0]?.id ?? null, focusedNodeIds: undefined }))
